...
export const GetAccomodatinType = async (dispatch, setOptions, payload = { depoCode: '' }) => {
    const action = await dispatch(fetchGetPreData({ dataInfo: payload, indicatorsPath: '/license/accommodation/null' }));
    if (fetchGetPreData.fulfilled.match(action) && action?.payload?.success?.length > 0) {
        const newUpted = action.payload.success.map((item) => ({
            ...
            label: `${item?.accommodationTypeDesc}`,
            items: item
        }))
