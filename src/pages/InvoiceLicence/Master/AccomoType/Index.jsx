...
const Index = () => {
    ...
    const handleSearchResults = async (p) => {
        ...
        const dataInfo = {
            ...(p.accomodationType && { accomodationType: p.accomodationType }),
            ...(p.accomodationTypeDesc && { accomodationTypeDesc: p.accomodationTypeDesc })
        }
        setPagination(dataInfo)
        try {
            const response = await dispatch(searchPostGetAction({ dataInfo: dataInfo, indicatorsPath: '/accommodation-type/search' }));
    ...
    const validateForm = (callback) => {
        let errors = {};
        if (!formData.accomodationType) errors.accomodationType = "Field is required.";
        if (!formData.accomodationTypeDesc) errors.accomodationTypeDesc = "Field is required.";
    ...
    const handleAddSubmit = useCallback(async (event) => {
        ...
        validateForm(async (isValid, errors) => {
            let indicatorsPath = '/accommodation-type/add';
            if (editPopupVisible) {
                indicatorsPath = '/accommodation-type/update';
            }
            if (isValid) {
                ...
                try {
                    ...
                    if (submitPostData.fulfilled.match(action)) {
                        if (action?.payload?.success) {
                            ...
                            if (editPopupVisible) {
                                setSuccessReponse('Accomodation Type updated successfully.')
                            } else {
                                setSuccessReponse('Accomodation Type save successfully.')
                            }
                            setTimeout(() => {
                                setAddPopupVisible(false);
                                setEditPopupVisible(false);
    ...
    return (
        <section>
            <BreadCrumb item={{ label: 'Invoicing-License Fee', 'sub_label': 'Master', active: true, current: 'Accomodation Type' }} />
