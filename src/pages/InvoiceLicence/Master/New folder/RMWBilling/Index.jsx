...
const Index = () => {
    ...
    const handleSearchResults = async (p) => {
        ...
        const dataInfo = {
            ...
            ...(p.waterCatDesc && { waterCatDesc: p.waterCatDesc }),
        }
        setPagination(dataInfo)
        try {
            const response = await dispatch(searchPostGetAction({ dataInfo: dataInfo, indicatorsPath: '/water-rate/search' }));
    ...
    const validateForm = (callback) => {
        ...
        if (!formData.waterCatDesc) errors.waterCatDesc = "Field is required.";
        ...
        if (!formData.rateAmt) errors.rateAmt = "Field is required.";
    ...
    const handleAddSubmit = useCallback(async (event) => {
        ...
        validateForm(async (isValid, errors) => {
            ...
            if (isValid) {
                ...
                try {
                     if (editPopupVisible) {
                        const indicatorsPath = '/water-rate/update';
                        ...
                        if (submitPutData.fulfilled.match(action)) {
                            ...
                            setSuccessReponse('Water rate updated successfully.')
                            setEditPopupVisible(false);
                        } else {
                            setSerMessage(action?.payload?.error)
                        }
                    } else {
                        const indicatorsPath = '/water-rate/add';
                        ...
                        if (submitPostData.fulfilled.match(action)) {
                            handleSearchResults(pagination)
                            setSuccessReponse('Water rate created successfully.')
                            setAddPopupVisible(false);
                        } else {
                            setSerMessage(action?.payload?.error)
                        }
    ...
    return (
        <section>
            <BreadCrumb item={{ label: 'Invoicing-Water', 'sub_label': 'Master', active: true, current: 'Rate Master' }} />
            <div className="custome-border ">
                ...
                >
                    <SearchInputField label="Water Cat Code" name="waterCatCode" inputData={initialFilterData.waterCatCode} onChange={filterHandleChange} />
                    <SearchInputField label="Water Cat Desc" name="waterCatDesc" inputData={initialFilterData.waterCatDesc} onChange={filterHandleChange} />
