...
const Index = () => {
    ...
    const handleSearchResults = async (p) => {
        ...
        const dataInfo = {
            ...(p.connectionTypeDesc && { connectionTypeDesc: p.connectionTypeDesc }),
            ...(p.connectionTypeCode && { connectionTypeCode: p.connectionTypeCode })
        }
        setPagination(dataInfo)
        try {
            const response = await dispatch(searchPostGetAction({ dataInfo: dataInfo, indicatorsPath: '/connection-type/search' }));
    ...
    const validateForm = (callback) => {
        let errors = {};
        if (!formData.connectionTypeDesc) errors.connectionTypeDesc = "Field is required."; 
    ...
    const handleAddSubmit = useCallback(async (event) => {
        ...
        validateForm(async (isValid, errors) => {
            let indicatorsPath = '/connection-type/add';
            if (editPopupVisible) {
                indicatorsPath = '/connection-type/update';
            }
            if (isValid) {
                ...
                try {
                    ...
                    if (submitPostData.fulfilled.match(action)) {
                        if (action?.payload?.success) {
                            ...
                            if (editPopupVisible) {
                                setSuccessReponse('Connection type updated successfully.')
                            } else {
                                setSuccessReponse('Connection type created successfully.')
                            }
                            setTimeout(() => {
                                setAddPopupVisible(false);
                                setEditPopupVisible(false);
    ...
    return (
        <section>
            <BreadCrumb item={{ label: 'Invoicing-Water', 'sub_label': 'Master', active: true, current: 'Connection Type Master' }} />
            <div className="custome-border ">
                ...
                >
                    <SearchInputField label="Connection Type Desc" name="connectionTypeDesc" inputData={initialFilterData.connectionTypeDesc} onChange={filterHandleChange} />
                    {/* <SearchInputField label="Connection Type Code" name="connectionTypeCode" inputData={initialFilterData.waterCatDesc} onChange={filterHandleChange} /> */}
