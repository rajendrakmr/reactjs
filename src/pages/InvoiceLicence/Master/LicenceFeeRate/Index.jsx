...
const Index = () => {
    ...
    const handleSearchResults = async (p) => {
        ...
        const dataInfo = {
            ...(p.accommodationType && { accommodationType: p.accommodationType }),
            ...(p.fromNo && { fromNo: p.fromNo }),
            ...(p.toNo && { toNo: p.toNo }),
            ...(p.accomCategory && { accomCategory: p.accomCategory })
        }
        setPagination(dataInfo)
        try {
            const response = await dispatch(searchPostGetAction({ dataInfo: dataInfo, indicatorsPath: '/license/fee/search' }));
    ...
    const validateForm = (callback) => {
        let errors = {};
        if (!formData.accommodationType) errors.accommodationType = "Field is required.";
        if (!formData.fromNo) errors.fromNo = "Field is required.";
        if (!formData.toNo) errors.toNo = "Field is required.";
        if (!formData.effectiveFromDate) errors.effectiveFromDate = "Field is required.";
        if (!formData.effectiveToDate) errors.effectiveToDate = "Field is required.";
        if (!formData.rentRateAmt) errors.rentRateAmt = "Field is required.";
        if (!formData.accomCategory) errors.accomCategory = "Field is required.";
        setErrors(errors);
    ...
    const handleAddSubmit = useCallback(async (event) => {
        ...
        validateForm(async (isValid, errors) => {
            let indicatorsPath = '/license/fee/add';
            if (editPopupVisible) {
                indicatorsPath = '/license/fee/update';
            }
            if (isValid) {
                ...
                try {
                    ...
                    if (editPopupVisible) { 
                        ...
                        if (submitPutData.fulfilled.match(action)) {
                            ...
                            setSuccessReponse('Success: License fee rate updated.')
                            setEditPopupVisible(false);
                        } else {
                        ...
                        if (submitPostData.fulfilled.match(action)) {
                            handleSearchResults(pagination)
                            setSuccessReponse('Success: License fee rate created.')
                            setAddPopupVisible(false);
                        } else {
                            setSerMessage(action?.payload?.error)
                        }
    ...
    return (
        <section>
            <BreadCrumb item={{ label: 'Invoicing-Water', 'sub_label': 'Master', active: true, current: 'Licence Fee Rate Master' }} />
            <div className="custome-border ">
                ...
                >
                    <SelectFormInput
                        ...
                        value={initialFilterData.accommodationType}
                    ...
                    <SelectFormInput
                        ...
                        id="accomCategory"
                        name="accomCategory"
                        value={initialFilterData.accomCategory}
                        options={accomodationCategory}
                        label="Accomm. Category"
                        col="col-md-4"
                    />

                    <SearchInputField label="From No" name="fromNo" inputData={initialFilterData.fromNo} onChange={filterHandleChange} />
                    <SearchInputField label="To No" name="toNo" inputData={initialFilterData.toNo} onChange={filterHandleChange} />
