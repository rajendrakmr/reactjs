...
const Index = () => {
    ...
    const handleSearchResults = async (p) => {
        ...
        try {
            const response = await dispatch(searchPostGetAction({ dataInfo: dataInfo, indicatorsPath: '/water-cat/search' }));
    ...
    const validateForm = (callback) => {
        ...
        if (!formData.gstApplicable) errors.gstApplicable = "Field is required.";
    ...
    const handleAddSubmit = useCallback(async (event) => {
        ...
        validateForm(async (isValid, errors) => {
            let indicatorsPath = '/water-cat/add';
            if (editPopupVisible) {
                indicatorsPath = '/water-cat/update';
            }
            if (isValid) {
                ...
                try {
                    ...
                    if (submitPostData.fulfilled.match(action)) {
                        if (action?.payload?.success) {
                            ...
                            if (editPopupVisible) {
                                setSuccessReponse('Water Catergory updated successfully.')
                            } else {
                                setSuccessReponse('Water Catergory save successfully.')
                            }
                            setTimeout(() => {
                                setAddPopupVisible(false);
                                setEditPopupVisible(false);
    ...
    return (
        <section>
            <BreadCrumb item={{ label: 'Invoicing-Water', 'sub_label': 'Master', active: true, current: 'Water Category Master' }} />
