...
const Index = () => { 
    ...
    const handleSearchResults = async (p) => { 
        ...
        try {
            const response = await dispatch(searchPostGetAction({ dataInfo: dataInfo, indicatorsPath: '/customer/search' }));
    ...
    const handleEditPopupOpen = (item) => {
        ...
        const dataInfo =
        {
            ...
            customerAddresses: respoDat?.customerAddresses.map(row => ({
                ...row,
                srlNo: row.addSrlNo
            })),
    ...
    const validateForm = (callback) => {
        ...
        if(formData.mailId && formData.mailId!==""){
        ...
        formData.customerAddresses.forEach((item, index) => {
            let itemErrors = {};
            ...
            if (Object.keys(itemErrors).length > 0) {
                errors[`customerAddresses_${index}`] = itemErrors;
            }
        });
    ...
    const handleAddSubmit = useCallback(async (event) => {
        ...
        const dataInfo = {
            ...
            customerAddresses: formData.customerAddresses.every(isObjectEmpty)
                ? []
                : formData.customerAddresses.map(row => ({
                    ...row,
        ...
        validateForm(async (isValid, errors) => { 
            console.log('errorserrors',errors,dataInfo)
            if (isValid) { 
                try {

                    if (editPopupVisible) {
                        const indicatorsPath = '/customer/update';
                        ...
                        const indicatorsPath = '/customer/add';
    ...
    return (
        <section>
            <BreadCrumb item={{ label: 'Invoicing-Water', 'sub_label': 'Transaction', active: true, current: 'Customer Master' }} />
            <div className="custome-border ">
                ...
                >
                    ...
                    <SelectFormInput
                        ...
                        col="col-md-4"
                    />
                    <SearchInputField label="GSTN" name="gstn" inputData={initialFilterData.gstn} onChange={filterHandleChange} />
                    <SearchInputField label="Phone No" name="phoneNo" inputData={initialFilterData.phoneNo} onChange={filterHandleChange} />
