...
const ChildEntryScreen = ({ customerAddresses, setFormData,hasError,formData, errors, setErrors, isEdit, stateOptions }) => {
  ...
  const handleAddRow = () => {
    ...
    setFormData(prevData => ({
      ...prevData,
      customerAddresses: [...prevData.customerAddresses, newRow]
    }));
  };
  ...
  const handleRemoveParent = (parentIndex) => {
    const updatedIndentItemList = customerAddresses.filter((_, index) => index !== parentIndex);
    setFormData(prevData => ({
      ...prevData,
      customerAddresses: updatedIndentItemList
    }));
  };
  ...
  const setError = useCallback((parentIndex, field) => {
    if (parentIndex !== undefined && field !== undefined) {
      return errors[`customerAddresses_${parentIndex}`] || '';
    }
  ...
  const setSelectError = useCallback((parentIndex, field) => {
    if (parentIndex !== undefined && field !== undefined) {
      return errors[`customerAddresses_${parentIndex}`]?.[field] || '';
    }
    return errors;
  }, [errors]);
  ...
  const handleChangeSelect = useCallback(async (e, parentIndex, field) => {
    ...
    setFormData(prevData => {
      const updatedIndentItemList = [...prevData.customerAddresses];
      updatedIndentItemList[parentIndex] = {
        ...items,
        ...updatedIndentItemList[parentIndex],
        ...setItem,
      };
      return { ...prevData, customerAddresses: updatedIndentItemList };
    });

    setErrors(prevErrors => {
      const updatedErrors = { ...prevErrors };
      const errorKey = `customerAddresses_${parentIndex}`;
  ...
  const handleCheckboxChange = (e, parentIndex, row) => {
    ...
    const updateCustomerDtl = [...customerAddresses];
    ...
    setFormData(prev => {
      return {
        ...prev,
        customerAddresses: updateCustomerDtl
      };
    });
        ...
        <div className="table-responsive scroll-container" style={{ maxHeight: "40vh", overflowY: "auto" }}>
          <table className="table table-bordered">
            ...
            <tbody className="child_item">
              {customerAddresses.length > 0 && customerAddresses?.map((row, parentIndex) => (
...
const areEqual = (prevProps, nextProps) => {
  return (prevProps.customerAddresses === nextProps.customerAddresses && prevProps.errors === nextProps.errors && prevProps.stateOptions === nextProps.stateOptions && prevProps.itemOptions === nextProps.itemOptions);
};
