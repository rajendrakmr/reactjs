...
const Index = () => {
    ...
    const handleSearchResults = async (p) => { 
        ...
        const dataInfo = {
            ...
            ...(p.consumerTypeDesc && { consumerTypeDesc: p.consumerTypeDesc })
        }
        setPagination(dataInfo)
        try {
            const response = await dispatch(searchPostGetAction({ dataInfo: dataInfo, indicatorsPath: '/consumer-type/search' }));
    ...
    const validateForm = (callback) => {
        let errors = {};
        if (!formData.consumerTypeDesc) errors.consumerTypeDesc = "Field is required."; 
    ...
    const handleAddSubmit = useCallback(async (event) => {
        ...
        validateForm(async (isValid, errors) => {
            let indicatorsPath = '/consumer-type/add';
            if (editPopupVisible) {
                indicatorsPath = '/consumer-type/update';
            }
            if (isValid) {
                ...
                try {
                    ...
                    if (submitPostData.fulfilled.match(action)) {
                        if (action?.payload?.success) {
                            ...
                            if (editPopupVisible) {
                                setSuccessReponse('Consumer type updated successfully.')
                            } else {
                                setSuccessReponse('Consumer type created successfully.')
                            }
                            setTimeout(() => {
                                setAddPopupVisible(false);
                                setEditPopupVisible(false);
    ...
    return (
        <section>
            <BreadCrumb item={{ label: 'Invoicing - Water', 'sub_label': 'Master', active: true, current: 'Consumer Type Master' }} />
            <div className="custome-border ">
                ...
                >
                    <SearchInputField label="Consumer Type Desc" name="consumerTypeDesc" inputData={initialFilterData.consumerTypeDesc} onChange={filterHandleChange} />
