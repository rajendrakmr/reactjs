...
import {GetFinanceYear, GetReportType } from "../apiService"; 
...
const FSNAReport = () => {
    ...
    const [fsnOptions, setFsnptions] = useState([]);
    ...
    useEffect(() => {
        ...
        GetFinanceYear(dispatch, setReportFYearOptions)
    ...
    const validateForm = (callback) => {
        let errors = {};
        if (!formData.finYear) errors.finYear = "Field is required.";
        if (!formData.itemType) errors.itemType = "Field is required.";
    ...
    const handleChangeSelect = useCallback((e, field) => {
        ...
        setFormData((prevFormData) => ({
            ...prevFormData,
            [field]: value,
            ...(field === "finYear" && { itemType: null, fromDate:null, toDate:null })
        }));
        setErrors(prevErrors => {
            const updatedErrors = { ...prevErrors };
    ...
    return (
        <section >
            <div className="container mt-5">
                <BreadCrumb item={{ label: 'Store', 'sub_label': 'Report', active: true, current: 'FSN Analysis' }} />
                <div className="pb-4">
                    ...
                    <div className="_rkContentBorder row py-2">
                        <div className="row mx-auto mw-100">
                            <SelectFormInput
                                onChange={(selectedOption) => handleChangeSelect(selectedOption, 'finYear')}
                                error={setSelectError('finYear')}
                                id="finYear"
                                name="finYear"
                            ...
                            <SelectFormInput
                                onChange={(selectedOption) => handleChangeSelect(selectedOption, 'itemType')}
                                error={setSelectError('itemType')}
                                id="itemType"
                                name="itemType"
