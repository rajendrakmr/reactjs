...
import { GetReportType, GetRecordType } from "../apiService";
...
const Index = () => {
    ...
    const [reportRTOption, setReportRTOptions] = useState([]);
    ...
    useEffect(() => { 
        GetRecordType(dispatch, setReportRTOptions)
    }, []);
    ...
    const validateForm = (callback) => {
        let errors = {};
        if (!formData.recType) errors.recType = "Field is required.";
    ...
    return (
        <section >
            <div className="container mt-5">
                <BreadCrumb item={{ label: 'Store', 'sub_label': 'Report', active: true, current: 'Pending Transaction Register' }} />
                <div className="pb-4">
                    <div className="align-items-center primary-light-bg text-white p-1 text-bold text-center position-relative">
                        <h5>Pending Transaction Register</h5>
                    </div>
                    <MandatoryField />
                    {serError && (
                    ...
                    <div className="_rkContentBorder row py-2">
                        <div className="row mx-auto mw-100">
                            <SelectFormInput
                                onChange={(selectedOption) => handleChangeSelect(selectedOption, 'recType')}
                                error={setSelectError('recType')}
                                id="recType"
                                name="recType"
                                value={formData.recType}
                                options={reportRTOption}
                                label="Record Type"
                                required={true}
                            />
