...
const ChildEntryScreen = ({ challanIssueChildren, setFormData,formData, errors, setErrors, isEdit, itemOptions, setItemCodeOptions }) => {
  ...
  const handleChange = useCallback((e, parentIndex, field) => {
    ...
    setFormData(prevData => {
      ...
      if(field=="challanIssueQty"){
        updatedIndentItemList[parentIndex] = {
          ...updatedIndentItemList[parentIndex], 
          amount: updatedIndentItemList[parentIndex].rate*value,
          requistionQty:value,
              ...
              {challanIssueChildren.length > 0 && challanIssueChildren.map((row, parentIndex) => (
                <React.Fragment key={parentIndex}>
                  <tr>
                    ...
                    <InputCellField value={row.challanIssueQty} onChange={(e) => handleChange(e, parentIndex, 'challanIssueQty')} name="challanIssueQty" errorMsg={setError(parentIndex, 'challanIssueQty')} className="wide-input1" type="number" max="10" />


                    <InputCellField value={row.rate} onChange={(e) => handleChange(e, parentIndex, 'rate')} name="rate" type="number" max="10" className="wide-input0" disabled={true} />
                    <InputCellField value={row.amount} onChange={(e) => handleChange(e, parentIndex, 'amount')} name="amount" errorMsg={setError(parentIndex, 'amount')} className="wide-input1" type="number" max="10" disabled={true} />
                    <InputCellField value={row.location} onChange={(e) => handleChange(e, parentIndex, 'location')} name="location" errorMsg={setError(parentIndex, 'location')} className="wide-input3"   disabled={true} />
...
const areEqual = (prevProps, nextProps) => {
  return (prevProps.formData.challanIssueChildren === nextProps.formData.challanIssueChildren && prevProps.errors === nextProps.errors && prevProps.itemOptions === nextProps.itemOptions);
};
