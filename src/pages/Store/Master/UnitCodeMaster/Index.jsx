...
const Index = () => {
    ...
    const handleSearchResults = async (p) => {
        ...
        const dataInfo = {
            ...
            ...(p.unitDescription && { unitDescription: p.unitDescription }),
            pageNo: p.page ? p.page : 0,
            pageSize: 10
        }
        setPagination(dataInfo)
        try {
            const response = await dispatch(searchPostGetAction({ dataInfo: dataInfo, indicatorsPath: '/unit-code/search' }));
    ...
    const validateForm = (callback) => {
        let errors = {};
        if (!formData.unitCode) errors.unitCode = "Unit code is required.";
        if (!formData.unitDescription) errors.unitDescription = "Unit Description is required.";
        ...
        validateForm(async (isValid, errors) => {
            if (isValid) {
                ...
                try {
                    ...
                    if (editPopupVisible) {
                        const indicatorsPath = '/unit-code/update';
                        ...
                        if (submitPutData.fulfilled.match(action)) {
                            ...
                            setSuccessReponse('Unit Code updated successfully.')
                            setEditPopupVisible(false);
                        } else {
                            setSerMessage(action?.payload?.error)
                        }
                    } else {
                        const indicatorsPath = '/unit-code/add';
                        ...
                        if (submitPostData.fulfilled.match(action)) {
                            handleSearchResults(pagination)
                            setSuccessReponse('Unit Code created successfully.')
                            setAddPopupVisible(false);
                        } else {
                            setSerMessage(action?.payload?.error)
                        }
    ...
    return (
        <section>
            <BreadCrumb item={{ label: 'Store', 'sub_label': 'Master', active: true, current: 'Unit Code Master' }} />
            <div className="custome-border ">
                ...
                >
                    <SearchInputField label="Unit Code" name="unitCode" inputData={initialFilterData.unitCode} onChange={filterHandleChange} />
                    <SearchInputField label="Unit Desc" name="unitDescription" inputData={initialFilterData.unitDescription} onChange={filterHandleChange} />
