...
const Index = () => {
    ...
    const handleSearchResults = async (p) => {
        ...
        const dataInfo = {
            ...(p.depoCode && { code: p.depoCode }),
            ...(p.depoDescription && { description: p.depoDescription })
        }
        setPagination(dataInfo)
        try {
            const response = await dispatch(searchPostGetAction({ dataInfo: dataInfo, indicatorsPath: '/dest/search' }));
    ...
    const validateForm = (callback) => {
        let errors = {};
        if (!formData.depoCode) errors.depoCode = "Deopt code is required.";
        if (!formData.depoDescription) errors.depoDescription = "Description code is required.";
        if (!formData.isActive) errors.isActive = "Status is required.";
        if (!formData.moduleFlag) errors.moduleFlag = "Flag is required.";
        ...
        validateForm(async (isValid, errors) => {
            if (isValid) {
                ...
                try {
                    ...
                    if (editPopupVisible) {
                        const indicatorsPath = '/dest/update';
                        ...
                        if (submitPutData.fulfilled.match(action)) {
                            ...
                            setSuccessReponse('Depo Master updated successfully.')
                            setEditPopupVisible(false);
                        } else {
                            setSerMessage(action?.payload?.error)
                        }
                    } else {
                        const indicatorsPath = '/dest/add';
                        ...
                        if (submitPostData.fulfilled.match(action)) {
                            handleSearchResults(pagination)
                            setSuccessReponse('Depo Master created successfully.')
                            setAddPopupVisible(false);
                        } else {
                            setSerMessage(action?.payload?.error)
                        }
    ...
    return (
        <section>
            <BreadCrumb item={{ label: 'Store', 'sub_label': 'Master', active: true, current: 'Depo Master' }} />
