...
const Index = () => {
    ...
    const handleSearchResults = async (p) => {
        const dataInfo = {
            ...
            ...(p.jobCode && { jobCode: p.jobCode }),
            ...(p.jobDescription && { jobDescription: p.jobDescription }),
            pageNo: p.page ? p.page : 0,
            pageSize: 10
        }
        setPagination(dataInfo)
        try {
            const response = await dispatch(searchPostGetAction({ dataInfo: dataInfo, indicatorsPath: '/depowise/search-job' }));
    ...
    const validateForm = (callback) => {
        ...
        if (!formData.jobCode) errors.jobCode = "Job code is required.";
        ...
        validateForm(async (isValid, errors) => {
            if (isValid) {
                ...
                try {
                    ...
                    if (editPopupVisible) {
                        const indicatorsPath = '/depowise/update-job';
                        ...
                        const indicatorsPath = '/depowise/add-job';
    ...
    return (
        <section>
            <BreadCrumb item={{ label: 'Store', 'sub_label': 'Master', active: true, current: 'Depo Wise Job Master' }} />
            <div className="custome-border ">
                ...
                >
                    <SelectFormInput
                        ...
                        col="col-md-4"
                    />
                    <SearchInputField label="Job Code" name="jobCode" inputData={initialFilterData.jobCode} onChange={filterHandleChange} />
                    <SearchInputField label="Job Desc" name="jobDescription" inputData={initialFilterData.jobDescription} onChange={filterHandleChange} />
