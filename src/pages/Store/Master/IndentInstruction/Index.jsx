...
const Index = () => {
    ...
    const handleSearchResults = async (p) => {
        ...
        const dataInfo = {
            ...(p.srlNo && { SrlNo: p.srlNo }),
            ...(p.indentDesc && { IndentDesc: p.indentDesc }),
            pageNo: p.page ? p.page : 0,
            pageSize: 10
        }
        setPagination(dataInfo)
        try {
            const response = await dispatch(searchPostGetAction({ dataInfo: dataInfo, indicatorsPath: '/indent-inst/search' }));
    ...
    const validateForm = (callback) => {
        let errors = {};
        if (!formData.srlNo) errors.srlNo = "Sr.No is required.";
        if (!formData.instructDesc) errors.instructDesc = "Description is required.";
        ...
        validateForm(async (isValid, errors) => {
            if (isValid) {
                ...
                try {
                    ...
                    if (editPopupVisible) {
                        const indicatorsPath = '/indent-inst/update';
                        ...
                        if (submitPutData.fulfilled.match(action)) {
                            ...
                            setSuccessReponse('Indent Instruction updated successfully.')
                            setEditPopupVisible(false);
                        } else {
                            setSerMessage(action?.payload?.error)
                        }
                    } else {
                        const indicatorsPath = '/indent-inst/add';
                        ...
                        if (submitPostData.fulfilled.match(action)) {
                            handleSearchResults(pagination)
                            setSuccessReponse('Indent Instruction created successfully.')
                            setAddPopupVisible(false);
                        } else {
                            setSerMessage(action?.payload?.error)
                        }
    ...
    return (
        <section>
            <BreadCrumb item={{ label: 'Store', 'sub_label': 'Master', active: true, current: 'Indent Instruction' }} />
            <div className="custome-border ">
                ...
                >
                    <SearchInputField label="SRL Number" name="srlNo" inputData={initialFilterData.srlNo} onChange={filterHandleChange} />
                    <SearchInputField label="Description" name="indentDesc" inputData={initialFilterData.indentDesc} onChange={filterHandleChange} />
